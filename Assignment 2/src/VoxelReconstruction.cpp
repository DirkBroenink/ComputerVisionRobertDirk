		...
		_data_path(dp), _cam_views_amount(cva)
{
	...
	for (int v = 0; v < _cam_views_amount; ++v)
	{
		...
		string bkimg = full_path.str() + General::BackgroundImageFile;
		string bkvid = full_path.str() + General::BackgroundVideoFile;

		assert(
			(General::fexists(bkimg) || General::fexists(bkvid))
			&&
		...
		string cfgfile = full_path.str() + General::ConfigFile;
		string intfile = full_path.str() + General::IntrinsicsFile;
		string chkbord = full_path.str() + General::CheckerboadVideo;

		assert(
			(General::fexists(cfgfile) ?
				General::fexists(intfile) &&
					General::fexists(chkbord)
			 : true)
		);

...
bool Camera::initialize()
{
	...
	_frames = (long) _video.get(CV_CAP_PROP_POS_FRAMES);
	assert(_frames > 1);
		...
		const string &out_fname)
{
	...
	FileStorage fs;
	string openFile = data_path + ".." + string(PATH_SEP) + General::CBConfigFile;
	cout << "Opening file: " << openFile;
	fs.open(openFile, FileStorage::READ);
	if (fs.isOpened())
	{
	...
	_BoardCorners = new vector<Point>(); //A pointer because we need access to it from static function onMouse

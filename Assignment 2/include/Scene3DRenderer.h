...
struct AdjustedFrame
{
	...
	AdjustedFrame(cv::Mat& aMat, int aIndex)
	{
		_Mat = aMat;
		_Index = aIndex;
		_Changed = false;
...
class Scene3DRenderer
{
	...
	int _before_erosion_threshold;
	int _erosion_threshold;
	int _mode;
	int _brush_size;
	bool _pixelsWereModified;
	bool _findNewOptimalHSVValues;

	std::vector<AdjustedFrame*> _adjusted_frames;

	// edge points of the virtual ground floor grid
	std::vector<std::vector<cv::Point3i*> > _floor_grid;

	void createFloorGrid();
	void FindNewOptimalHSVValues();
	void ColorDesiredForegroundValue(int frame, int x, int y, byte color);
	void ColorNeighbouringPixels(cv::Mat& _mat, int x, int y, byte color);
	bool HasAdjustedFrame(int frame);
	void AddAdjustedFrame(int aframe);
	AdjustedFrame* GetAdjustedFrame(int frame);
	void CalcHSVFilteredForeground(Camera* aCamera, std::vector<cv::Mat>& aChannels, cv::Mat& aDst, char H, char S, char V);
